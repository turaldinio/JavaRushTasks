...
public class Model {
    ...
    public void randomMove() {
        int randomCount = (int) (((Math.random() * 100)) % 4);
        switch (randomCount) {
            case 0:
                left();
                break;
    ...
    public boolean hasBoardChanged() {
        ...
        for (int a = 0; a < tmp.length; a++) {
            for (int b = 0; b < tmp[a].length; b++) {
                sumFirstArray += gameTiles[a][b].getValue();
                sumSecondArray += tmp[a][b].getValue();
            }
        }
    ...
    private MoveEfficiency getMoveEfficiency(Move move) {
        move.move();
        MoveEfficiency moveEfficiency = new MoveEfficiency(getEmptyTilesCount(), score, move);
        if (!hasBoardChanged()) {
